function [huberresult]=totalvar(s,W,nrofim);
...
while iter < max_iter
     initialvector(:,i)=initialvector(:,i)-0.001*W'*(W*initialvector(:,i)-lowresvec)+(I-S{2})*sign(initialvector(:,i)-S{3}*initialvector(:,i));
     iter=iter+1;
end
sr(:,:,i)=reshape(real(initialvector(:,i)),col,row)';
huberresult2(:,:,i) = medfilt2(sr(:,:,i),[3 3]);
huberresult(:,:,i)=huberresult2(2:end-1,2:end-1,i);
end
end